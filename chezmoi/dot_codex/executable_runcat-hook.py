...
OUT = Path(os.environ.get(
    "RUNCAT_CODEX_OUT_FILE",
    str(Path.home() / ".codex" / "runcat-usage.json"),
))
ICON = Path(os.environ.get(
    "RUNCAT_CODEX_ICON_FILE",
    str(Path.home() / ".codex" / "runcat-icon.svg"),
))
ICON_RENDERING_MODE = os.environ.get("RUNCAT_CODEX_ICON_RENDERING_MODE", "template")
if ICON_RENDERING_MODE not in {"original", "template"}:
    ICON_RENDERING_MODE = "template"
...
def custom_icon():
    ...
    return {
        ...
        "renderingMode": ICON_RENDERING_MODE,
    }
